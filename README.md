...
在一个测试用例中，如果多个接口间有依赖，如 详情接口与更新接口 依赖于新增接口（详情接口与更新接口都需要使用到 新增接口创建完数据生成的主键id），使用teststeps定义接口请求顺序，同时使用extract参数，就可从新增接口的 HTTP 请求的响应结果中提取参数，并保存到参数变量中（例如`welcomeId`），后续测试步骤可通过`$welcomeId`的形式进行引用
